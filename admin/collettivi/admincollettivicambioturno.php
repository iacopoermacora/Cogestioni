...
<?php include $_SERVER['DOCUMENT_ROOT'].'/main/basicpage/header.php'; ?>
 ...
 <title><?php echo $set['set_intestazione_sito']; ?> - Admin - Collettivi disposti a cambiare turno</title>
...
<?php include $_SERVER['DOCUMENT_ROOT'].'/admin/basicpage/navbar_admin.php' ?>
  ...
  <div class="container">
    ...
    <div style="border-radius: 15px; background-color: #fff;" class="row spazietto_sopra spazietto_sotto center z-depth-1 margine_sotto">
      <h5 class="col s12 spazietto_sotto spazietto_sopra"><b>Ecco i <?php echo $set['set_nome_collettivi']; ?> disponibili a cambiare turno<?php
  ...
  $sql = "SELECT * FROM collettivi WHERE NOT ".$turno." = -100 AND NOT disponibile_cambio_turno = 'NO' AND eliminato = 'NO' AND id > 0";
  ...
  while($row = mysqli_fetch_assoc($collettivi2)) {
    ...
    DisplayCard($row, 'id', '', '', '', $set, 3, 0, 0);
          ...
          ?>
        <?php }} else { ?>
          <p class="center margine_sopra">Mannaggia! Nessuno dei <?php $set['set_nome_collettivi']; ?> è disponibile a cambiare turno<?php
          for ($i=1; $i  <= $set['set_turni_totali'] ; $i++) {
