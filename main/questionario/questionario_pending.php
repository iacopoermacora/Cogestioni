...
<div class="hero_and_content">
    <section class="hero" style="background-color: white;">
            <div class="hero-inner">
              <div class="container">
                <h5 class="black-text">Stai cercando di compilare il questionario di gradimento?</h1>
                <h6 class="black-text"><?php if ($_SESSION['loggedin_coge'] == true) {
                  if($_SESSION['ruolo_coge'] == 'professore' || $_SESSION['ruolo_coge'] == 'professore_admin'){
                  echo "Non viene richiesto che i docenti compilino il form di gradimento. Grazie per la collaborazione.";
                  }
                } else {
                  echo "Per compilare il questionario devi prima fare il login con le credenziali della ".$set['set_nome_cogestione']." che hai scelto. Fai il <a href='/main/system/login'>Login</a>";
                }
                   ?></h6>
              </div>
            </div>
